...
- `context.ts` нормализует вопрос, ищет темы по алиасам, добавляет безопасные производные факты и отвечает детерминированно на частые HR-вопросы: возраст, дата рождения, локация, контакты, PDF, зарплатный ориентир, форматы работы, английский, актуальный статус, релокация, доступность, стаж, “сколько лет назад”, программирование, NDA-метрики и ключевые проекты.
- Основные функции слоя: `normalizeQuestion`, `detectDeterministicIntent`, `buildDerivedFacts`, `buildRelevantContext`.
- `buildDerivedFacts` считает точные вычисляемые значения на backend: текущую дату, возраст от `1989-11-23`, календарный период опыта с 2022 года, контакты, защищённые темы, форматы работы и зарплатный ориентир.
- Prompt запрещает Gemini самостоятельно считать возраст, стаж, даты, “сколько лет назад”, проценты роста метрик, уверенное программирование на Python/C#/SQL, неуказанный коммерческий опыт, доступность, сроки выхода, NDA-детали и уровень английского сверх базы.
