...
## Где менять слоты игровых прототипов

Секция `Proof-of-work` с якорем `#proof-game` сейчас показывает нейтральную витрину игровых прототипов. Header-ссылка `Мини игра` продолжает вести на эту секцию.
...
- `GamePrototypeHub.tsx` — UI витрины, карточки слотов и disabled CTA;
- `gamePrototypeSlots.ts` — список будущих прототипов, статусы и короткие описания.

Первый слот зарезервирован под `Стройка века`: puzzle-прототип про строительство советских ретро-футуристических зданий на процедурно сгенерированных полях. Сам прототип пока не реализован.
...
- Игровые прототипы пока представлены как слоты без backend, роутинга и сохранения прогресса.
