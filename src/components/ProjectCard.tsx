...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <article className="group flex h-full w-full min-w-0 max-w-full flex-col overflow-hidden rounded-xl border border-white/10 bg-slate-900/70 p-6 shadow-soft transition hover:border-teal-300/25 hover:bg-slate-900/85 sm:p-7">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <h3 className="break-words text-2xl font-semibold text-slate-50">{project.title}</h3>
          <p className="mt-2 max-w-xl break-words text-base font-medium leading-7 text-teal-300">
            {project.type}
          </p>
        </div>
        {project.link ? (
      ...
      {project.focus ? (
        <p className="mt-5 break-words text-[15px] leading-7 text-slate-400">
          <span className="font-semibold text-slate-200">Focus:</span> {project.focus}
        </p>
      ) : null}

      <p className="mt-5 break-words text-[17px] leading-8 text-slate-300">{project.description}</p>
      ...
      {project.analyticsNote ? (
        <p className="mt-6 break-words rounded-lg border border-amber-300/25 bg-amber-300/10 px-4 py-3 text-[16px] leading-7 text-amber-100">
          {project.analyticsNote}
        </p>
      ) : null}
    </article>
